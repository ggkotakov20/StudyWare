...
Body::Body() {
	...
	bodySize = sHeight / 300;
	organSize = sHeight / 300;
...
void Body::drawAlertForEnd() {
	...
	if (yesBtn.hovering && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
		...
		showResult = true;
		for (int i = 0; i < 10; i++) {
			if (i == 0) questionTurn[i] = true;
			else questionTurn[i] = false;
		}
	}
...
void Body::drawTestButtons(Questions question1[]) {
	if (!questionTurn[0]) {
		Button previousBtn;
	...
	if (showResult) {
		finishBtn.bounds = { (float)sWidth / 2 + sWidth / 5 + 40, (float)sHeight - 75, 190, 50 };
		finishBtn.text = "Finish review";
	}
	else {
	...
	if (CheckCollisionPointRec(GetMousePosition(), finishBtn.bounds) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
		if (inTest) {
			if (showResult){
				...
				for (int i = 0; i < 10; i++) {
					for (int j = 0; j < 4; j++) {
						question1[i].ans[j].checked = false;
					}
					if(i == 0)
						questionTurn[i] = true;
					else questionTurn[i] = false;
		...
		if (i == 0 && questionTurn[i]) {
			...
			for (int j = 0; j < 4; j++) {
				...
				case 0:
					...
					if (question1[i].ans[j].checked && !showResult) {
						DrawCircle(checkBoxPos.x, checkBoxPos.y, 10, DARKGRAY);
						DrawCircle(checkBoxPos.x, checkBoxPos.y, 7, BLUE);
					}
					else if (j == question1[i].currectAns && showResult) {
						DrawCircle(checkBoxPos.x, checkBoxPos.y, 10, DARKGRAY);
						DrawCircle(checkBoxPos.x, checkBoxPos.y, 7, GREEN);
					}
					else if (question1[i].ans[j].checked && j != question1[i].currectAns && showResult) {
						DrawCircle(checkBoxPos.x, checkBoxPos.y, 10, DARKGRAY);
...
void Body::drawLungs() {
	...
	if (lungsIsActive) {
		...
		cancelBtn(sWidth / 2 - body.width - 55, sHeight / 2 + body.height / 9);
		testBtn(sWidth / 2 - body.width + 75, sHeight / 2 + body.height / 9, TestFor::LUNGS);
...
void Body::drawLiver() {
	...
	if (liverIsActive) {
		...
		cancelBtn(sWidth / 2 - body.width - 55, sHeight / 2 + body.height / 20);
		testBtn(sWidth / 2 - body.width + 75, sHeight / 2 + body.height / 20, TestFor::LIVER);
...
void Body::drawHeart() {
	...
	if (heartIsActive) {
		...
		cancelBtn(sWidth / 2 - body.width - 55, sHeight / 2 + body.height / 10);
		testBtn(sWidth / 2 - body.width + 75, sHeight / 2 + body.height / 10, TestFor::HEART);
...
void Body::drawIntestines() {
	...
	if (intestinesIsActive) {
		...
		cancelBtn(sWidth / 2 - body.width - 55, sHeight / 2 + body.height / 6.8);
		testBtn(sWidth / 2 - body.width + 75, sHeight / 2 + body.height / 6.8, TestFor::INTESTINES);
...
void Body::drawNerves() {
	...
	if (nervesIsActive) {
		...
		cancelBtn(sWidth / 2 - body.width - 55, sHeight / 2 + body.height / 7);
		testBtn(sWidth / 2 - body.width + 75, sHeight / 2 + body.height / 7, TestFor::NERVES);
...
void Body::drawBody() {
	if (!inTest) {
		...
		case Body::TestFor::STOMACH:
			...
			questions[1].Questions::currectAns = 3;
